...
def generate_pr_title(
    ...
    pr_content = git_data.get("pr_content", "")  # Get PR content if available
    model = git_data.get(
        "model", DEFAULT_PR_AI_MODEL
    )  # Get model from git_data or use PR default
    ...
    if verbose:
        ...
        debug_item("PR content available", "Yes" if pr_content else "No")
        debug_item("Model", model)

    # Format the user prompt with the actual data
    if pr_content:
        # If PR content is available, use it for more accurate title generation
        prompt = ADVANCED_PR_TITLE_USER_PROMPT.format(
            commit_messages="\n".join(commit_messages),
            diff_text=pr_content,  # Use PR content instead of raw diff
        )
    else:
        # Otherwise fall back to using commit messages and diff
        prompt = ADVANCED_PR_TITLE_USER_PROMPT.format(
            commit_messages="\n".join(commit_messages),
    ...
    client = AIClient(
        {"verbose": verbose}, use_pr_model=True
    )  # Pass verbose flag in config
...
def generate_pr_summary(
    partial_pr_markdown: str, verbose: bool = False
) -> str:
    """
    ...
    """
    context = {
        "partial_pr_markdown": partial_pr_markdown,
    }

    prompt = ADVANCED_PR_SUMMARY_USER_PROMPT.format(
        partial_pr_markdown=partial_pr_markdown
    )
...
def generate_additional_notes(
    ...
    prompt = ADVANCED_ADDITIONAL_NOTES_USER_PROMPT.format(
...
def generate_pr_simple(git_data: Dict, verbose: bool = False) -> str:
    """
    Generate a compact pull request description in markdown format using a single
    AI request. The output is automatically formatted and cleaned up.

    Args:
        git_data: Dictionary with commit messages, diff, file changes, and ai-model.
        verbose: Whether to log debug information
    ...
    overview_text = diff_text[:10000] if len(diff_text) > 10000 else diff_text
    changes_overview = (
    ...
    prompt = SIMPLE_PR_USER_PROMPT.format(**prompt_args)
...
def review_final_pr(markdown: str, verbose: bool = False) -> str:
    ...
    try:
        ...
        # If review fails due to AI client errors, return the original markdown
        if verbose:
            debug_item("PR Review Error", str(e))
        return clean_markdown_formatting(markdown)
